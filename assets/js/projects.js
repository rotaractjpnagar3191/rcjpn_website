(function () {
  ...
  const csvPath = ((csvTemplate && (csvTemplate.textContent || "").trim()) || "assets/data/projects-2025-26.csv");
  ...
  const markInlineLinks = (root = document) => {
    root.querySelectorAll('a.glx[href^="#"]').forEach((a) => a.setAttribute("data-type", "inline"));
  };

  const buildCard = (p) => {
    const meta = [prettyDate(p.start_date), p.venue].filter(Boolean).join(" • ");
    const statusBadge =
      p.status === "completed" ? "success" : p.status === "ongoing" ? "warning" : p.status === "upcoming" ? "info" : "secondary";

    return `
      <div class="col-12 col-sm-6 col-lg-4 project"
           data-cat="${p.category}" data-status="${p.status}" data-title="${escapeHtml(p.name)}">
        <article class="project-card h-100">
          ...
          <div class="body">
            <div class="tags">
              ...
              <span class="badge text-bg-${statusBadge}">${p.status_label}</span>
            </div>
            <h3 class="h5 mt-2 mb-1">${escapeHtml(p.name)}</h3>
            <p class="meta small text-muted">${escapeHtml(meta)}</p>
            <p class="desc">${escapeHtml(p.description)}</p>
            <div class="actions">
              <a class="btn btn-primary btn-sm glx" href="#p-${p.slug}" data-type="inline">Details</a>
            </div>
          </div>
        </article>
      </div>`;
    ...
    return `
      <div id="p-${p.slug}" class="glightbox-desc">
        <div class="project-modal">
          ...
          <p class="text-muted small mb-3">${p.category_label} • ${p.status_label}</p>
          <img class="mb-3 rounded" src="${p.cover}" alt="">
          ${p.description ? `<p>${escapeHtml(p.description)}</p>` : ""}
          ${p.start_date ? `<p class="small text-muted mt-2">Date: ${prettyDate(p.start_date)}</p>` : ""}
          ${p.venue ? `<p class="small text-muted">Venue: ${escapeHtml(p.venue)}</p>` : ""}
        </div>
      </div>`;
  };
  ...
  const applyFilters = () => {
    ...
    items.forEach((el) => {
      ...
      const show = (CURRENT_CAT === "all" || cat === CURRENT_CAT) && (CURRENT_STATUS === "all" || status === CURRENT_STATUS);
  ...
  const wireFilters = () => {
    ...
    document.querySelectorAll(".project-controls [data-cat]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document.querySelectorAll(".project-controls [data-cat]").forEach((b) => b.classList.remove("active"));
    ...
    // Status buttons
    document.querySelectorAll(".project-controls [data-status]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document.querySelectorAll(".project-controls [data-status]").forEach((b) => b.classList.remove("active"));
        btn.classList.add("active");
  ...
  const loadCsv = async () => {
    try {
      ...
      PROJECTS = rows.map((r) => {
        const name = firstNonEmpty(r, ["Name", "Project Name", "Title"], "Untitled Project");
        const [category, category_label] = mapCategory(firstNonEmpty(r, ["Avenues of Service", "Avenue", "Category"], ""));
        const desc = firstNonEmpty(r, ["Description", "Brief", "Summary"], "");
        const start = firstNonEmpty(r, ["Start Date", "Date"], "");
        const venue = firstNonEmpty(r, ["Venue", "Location", "Place"], "");

        let cover = firstNonEmpty(r, ["Cover Photo", "Cover", "Image", "Photo"], "assets/img/placeholder-16x9.png");
        if (cover.includes(",") || cover.includes(";")) {
          cover = cover.split(/[;,]/)[0].trim();
        }

        const statusCsv = firstNonEmpty(r, ["Status"], "").toLowerCase();
        const status = ["upcoming", "ongoing", "completed"].includes(statusCsv) ? statusCsv : "completed";
        const status_label = status === "completed" ? "Completed" : status === "ongoing" ? "Ongoing" : "Upcoming";
